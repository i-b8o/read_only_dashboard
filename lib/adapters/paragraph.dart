...
class ParagraphAdapter {
  final masterClient = MasterClient();
    ...
    Future<ParagraphData?> get(int id) async {
      try {
        Int64 id64 = Int64(id);
        final GetOneParagraphResponse resp =
        await masterClient.paragraphStub.getOne(master_grpc_service.GetOneParagraphRequest(iD: id64));
        if (resp.content.isEmpty) {
          return null;
        }
        // Mapping
        ParagraphData paragraphData = ParagraphData(id: id , content: resp.content);
      ...
      try {
        Int64 id64 = Int64(id);
        final UpdateParagraphRequest req = UpdateParagraphRequest(iD: id64, content: content);
